...
var highChartConverter = (function ( $ ) {
  ...
  return  {
    ...
    convertCountYoySeries: function (data) {
      var countSeries = [];
...
var dataLoad = (function ( $ ) {
  return function(data) {
    ...
    terms = []
    $.map(data, function ( datapoint, i ) {
      terms = $.merge(terms, Object.keys( datapoint["terms"] ));
    })
    terms = $.unique(terms)
  
    // transform raw data in to format consumable by highcharts
    categories = highChartConverter.convertCategories(data);
    series = highChartConverter.convertSeries(data);
    ...
    $('#comments_chart').highcharts({
      ...
      series: highChartConverter.convertCountYoySeries(data)
    });
...
$(function () {
  $.ajax({
    url: data_path,
    success: function(data) { dataLoad(data) },
    dataType: 'json',
  })
})
